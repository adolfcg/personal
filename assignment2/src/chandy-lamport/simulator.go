...
type Simulator struct {
	...
	//snapshots is a table who maps an intkey with SnapshotState value*
	snapshots map[int]*SnapshotState
	// variables type SyncMap to verify the state of the Sync*
...
func NewSimulator() *Simulator {
	return &Simulator{
		...
		NewLogger(),
		//initialize the tuple with make*
		make(map[int]*SnapshotState),
		// calls to New SyncMap*
		NewSyncMap(),
		NewSyncMap(),
	}
}
...
// Once the switch case is evaluetad*
// Run an event in the system SendTockens for send , Startsnap for snap
...
func (sim *Simulator) Tick() {
	...
	for _, serverId := range getSortedKeys(sim.servers) {
		...
		for _, dest := range getSortedKeys(server.outboundLinks) {
			...
			// We need to ensure there are messages in the events queue
			if !link.events.Empty() {
				...
				if e.receiveTime <= sim.time {
					...
					fmt.Println("Time:", e.receiveTime)
					link.events.Pop()
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	// TODO: IMPLEMENT ME
	// Generate a call of CL algorithm in server function
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	if ok2 {
		...
		if tot == len(sim.servers) {
			//sim.isSnapshotComplete[snapshotId] = true
